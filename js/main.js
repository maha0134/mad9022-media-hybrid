...
const PLAYER = {
  ...
  addListeners: () => {
    document
      ...
      .addEventListener("click", PLAYER.routing);
    document
      .querySelector(".unstyled-list")
      .addEventListener("click", PLAYER.clickedSong);
  ...
  btnMute: () => {
    ...
    let buttonMute = document.getElementById("btnMute");
    if (PLAYER.player.muted) {
      buttonMute.firstElementChild.textContent = "volume_up";
    } else {
      buttonMute.firstElementChild.textContent = "volume_off";
    }
  },
  ...
  replayTenSeconds: () => {
    if (!PLAYER.player.paused) {
      ...
      if (PLAYER.player.currentTime <= 10.0) {
        PLAYER.player.currentTime = 0.0;
      } else {
        PLAYER.player.currentTime -= 10.0;
      }
    } else {
      PLAYER.player.currentTime = 0.0;
    }
  ...
  forwardTenSeconds: () => {
    if (!PLAYER.player.paused) {
      ...
      if (PLAYER.player.duration - PLAYER.player.currentTime <= 10.0) {
  ...
  updateTotalTime: () => {
    ...
    let totalTime = PLAYER.convertIntoSeconds(
      Math.round(PLAYER.player.duration)
    ); //Math.round -- to get total seconds as an integer
  ...
  convertIntoSeconds: (duration) => {
    let minutes = Math.floor(duration / 60)
      .toString()
      .padStart(2, "0"); //minutes part padded with a zero
    let seconds = Math.floor(duration % 60)
      .toString()
      .padStart(2, "0"); //seconds part
